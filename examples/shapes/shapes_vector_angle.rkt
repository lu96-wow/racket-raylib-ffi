...
(let main-loop ([angle-mode 0])
  (unless (window-should-close?)
    (ptr-set! v2 _float 0 (exact->inexact (get-mouse-x)))
    (ptr-set! v2 _float 1 (exact->inexact (get-mouse-y)))

    (define next-mode (if (is-key-pressed KEY-SPACE) (- 1 angle-mode) angle-mode))

    (when (and (= next-mode 0) (is-mouse-button-down MOUSE-BUTTON-RIGHT))

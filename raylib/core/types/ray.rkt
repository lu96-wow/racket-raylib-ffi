...
;; types/ray.rkt — Ray (24 bytes, pass-by-value)
...
(define (ray px py pz dx dy dz)
  (let ([r (malloc _Ray 'atomic)])
    (ptr-set! r _float 0 (exact->inexact px))
    (ptr-set! r _float 1 (exact->inexact py))
    (ptr-set! r _float 2 (exact->inexact pz))
    (ptr-set! r _float 3 (exact->inexact dx))
    (ptr-set! r _float 4 (exact->inexact dy))
    (ptr-set! r _float 5 (exact->inexact dz))
    r))
...
(provide _Ray _ray-bytes
         ray ray-pos-x ray-pos-y ray-pos-z ray-dir-x ray-dir-y ray-dir-z

...
;; types/bounding-box.rkt — BoundingBox (24 bytes, pass-by-value)
...
(define (bounding-box mnx mny mnz mxx mxy mxz)
  (let ([bb (malloc _BoundingBox 'atomic)])
    (ptr-set! bb _float 0 (exact->inexact mnx))
    (ptr-set! bb _float 1 (exact->inexact mny))
    (ptr-set! bb _float 2 (exact->inexact mnz))
    (ptr-set! bb _float 3 (exact->inexact mxx))
    (ptr-set! bb _float 4 (exact->inexact mxy))
    (ptr-set! bb _float 5 (exact->inexact mxz))
    bb))
...
(provide _BoundingBox _bounding-box-bytes
         bounding-box

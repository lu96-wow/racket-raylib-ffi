...
;; types/camera3d.rkt — Camera3D (44 bytes, pass-by-value)
...
;; ═══════════════════════════════════════════════════════════
;; 分组访问器 — position/target/up 整体取/设
;; ═══════════════════════════════════════════════════════════
...
(define (bytes->camera3d lst)
  (let ([cam (malloc _Camera3D 'atomic)])
    ...
    (ptr-set! cam _float 6  (list-ref lst 6))
    (ptr-set! cam _float 7  (list-ref lst 7))
    (ptr-set! cam _float 8  (list-ref lst 8))
    (ptr-set! cam _float 9  (list-ref lst 9))
    (ptr-set! cam _int 10   (list-ref lst 10))
    cam))
...
(provide _Camera3D _camera3d-bytes
         camera3d
